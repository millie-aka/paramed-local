	...
	<table class="gpnf-nested-entries">
		...
		<tr data-bind="attr: { 'data-entryid': id }">
			...
			<td class="gpnf-row-actions" style="display: none;" data-bind="visible: true">
				<ul>
					<li class="edit"><button class="edit-button gform-theme-button--secondary" data-bind="click: $parent.editEntry, attr: { 'aria-label': '<?php echo esc_js( $aria_labels['edit_entry'] ); ?>'.gformFormat( $index() + 1, f<?php echo $nested_fields[0]['id']; ?>.label ) }"><?php echo $labels['edit_entry']; ?></button></li>
