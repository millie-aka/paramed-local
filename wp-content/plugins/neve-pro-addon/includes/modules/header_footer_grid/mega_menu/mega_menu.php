...
class Mega_Menu {
	...
	public function __construct() {
		...
		$this->colors_array = [
			'bgColor'      => __( 'Background Color', 'neve-pro-addon' ),
			'color'        => __( 'Text/Link Color', 'neve-pro-addon' ),
			'hoverColor'   => __( 'Link Hover Color', 'neve-pro-addon' ),
			'headingColor' => __( 'Headings Color', 'neve-pro-addon' ),
			'iconColor'    => __( 'Icons Color', 'neve-pro-addon' ),
			'borderColor'  => __( 'Border Color', 'neve-pro-addon' ),
		];
	...
	public function update_menu_item_data( \WP_REST_Request $request ) {
		...
		if ( ! in_array( $status, [ 'draft', 'publish' ], true ) ) {
			return new \WP_REST_Response(
				[
					'success' => false,
					'message' => __( 'Menu item does not exist.', 'neve-pro-addon' ),
				]
			);
		}
		...
		if ( ! is_array( $options ) ) {
			return new \WP_REST_Response(
				[
					'success' => false,
					'message' => __( 'Invalid data. Please reload the page and try again.', 'neve-pro-addon' ),
				]
			);
		}
		...
		if ( Loader::has_compatibility( 'track' ) ) {
			\Neve\Core\Tracker::track(
				array(
					array(
						'feature'          => 'mega-menu-pro',
						'featureComponent' => 'item-data',
	...
	private function get_individual_custom_layouts() {
		...
		$returnable = [ '-' => __( 'None', 'neve-pro-addon' ) ];
		...
		foreach ( $layouts['individual'] as $id => $priority ) {
			...
			if ( empty( $title ) ) {
				...
				$title = sprintf( esc_html__( 'Custom layout %d (No title)', 'neve-pro-addon' ), $id );
			}
	...
	public function add_menu_inline_styles( $css, $context ) {
		...
		foreach ( $items as $item ) {
			...
			$css .= '.layout-fullwidth .hfg-slot { position: relative }';
		}
