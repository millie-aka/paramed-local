...
class Manager {
	...
	public function pattern_styles() {
		...
		if ( 'neve_custom_layouts' === get_post_type() && $this->has_woocommerce() && ! $this->has_otter() ) {
			...
			$notice_text = sprintf( __( 'The WooCommerce Patterns from %1$s require %2$s installed and activated.', 'neve-pro-addon' ), wp_kses_post( $plugin_name_neve ), wp_kses_post( $plugin_name_otter ) );
			$script      = <<<JS
if ( window.wp ) {
    const wp = window.wp;
