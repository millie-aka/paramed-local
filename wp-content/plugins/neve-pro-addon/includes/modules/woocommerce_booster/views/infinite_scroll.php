...
class Infinite_Scroll {
	...
	private function result_count() {
		...
		if ( 1 === intval( $total ) ) {
			$markup .= __( 'Showing the single result', 'neve-pro-addon' );
		} elseif ( $total <= $per_page || -1 === $per_page ) {
			/* translators: %d: total results */
			$markup .= sprintf( _n( 'Showing all %d result', 'Showing all %d results', $total, 'neve-pro-addon' ), $total );
		} else {
			$first = 1;
			$last  = min( $total, $per_page * $current );
			/* translators: 1: first result 2: last result 3: total results */
			$markup .= sprintf( _nx( 'Showing %1$d&ndash;%2$d of %3$d result', 'Showing %1$d&ndash;%2$d of %3$d results', $total, 'with first and last result', 'neve-pro-addon' ), $first, $last, $total );
		}
		$markup .= '</p>';
