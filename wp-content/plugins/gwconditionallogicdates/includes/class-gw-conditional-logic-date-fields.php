...
if ( ! class_exists( 'GWConditionalLogicDateFields' ) ) {
	...
	class GWConditionalLogicDateFields {
		...
		/**
		 * @var null|array Holds the current form during conditional logic.
		 *
		 * @see GWConditionalLogicDateFields::set_current_form()
		 */
		...
		function __construct( $perk ) {
			...
			add_filter( 'gform_rule_source_value', array( $this, 'set_current_form' ), 10, 6 );
			...
			add_action( 'gform_register_init_scripts', array( $this, 'register_client_offset_init_script' ), 10, 2 );
			...
			<script type="text/javascript">
				if (window.gform) {
					...
					gform.addFilter('gform_conditional_logic_operators', function (operators, objectType, fieldId) {
						...
						if ( ! isDateTimeBasedRuleFieldId( fieldId ) ) {
							return operators;
						}
					...
					function isDateTimeBasedRuleFieldId( fieldId ) {

						if ( fieldId === '_gpcld_current_time' ) {
							return true;
						}
						...
						return inputType === 'date' || inputType === 'time';
					}
					...
					jQuery(document).ready(function ($) {
						...
						// Fix conditional logic flyout click for Date field.
						gform.addFilter( 'gform_conditional_logic_is_valid_flyout_click', function( isValidFlyoutClick, event ) {
							...
							return Boolean( $target.parents( '.gpcld-datepicker' ).length || $target.parents( '.ui-datepicker-calendar' ).length || $target.parents( '.ui-datepicker-header' ).length );
						} );

						function delayedRefreshInputs() {
							if ( refreshInputsTimeout ) {
								clearTimeout( refreshInputsTimeout );
							}
						...
						function initializeConditionalLogicValueInput($select) {
							...
							var value          = $select.val(),
								...
								$input         = $('#{0}_rule_value_{1}'.gformFormat(prefix, index)),
		...
		/**
		 * Set the current form, so we can use it in is_value_match()
		 * as is_value_match() does not pass the $form nor $entry.
		 *
		 * Requires GF 2.6.2 or newer.
		 *
		 ...
		 */
		function set_current_form( $source_value, $rule, $form, $logic, $entry ) {
		...
		function is_value_match( $is_match, $field_value, $target_value, $operator, $source_field, $rule ) {
			...
			if ( $rule['fieldId'] == '_gpcld_current_time' ) {
				...
				$base_date = rgar( $rule, 'gpcldBaseDate' );
				if ( $base_date ) {
					if ( is_numeric( $base_date ) ) {
						$target_value = strtotime( "+{$base_date} day", $target_value );
					} else {
						$target_value = strtotime( $base_date . ' ' . $rule['value'] );
					}
				}
			} else {
			...
			if (
				! empty( $this->current_form )
				&& $this->should_use_client_offset( $this->current_form['id'] )
				&& ! rgblank( rgpost( 'gpcld_visitor_gmt_offset' ) )
			) {
				$site_tz = wp_timezone();

				// Create a DateTime object using the current value and the site's timezone.
				$datetime = new DateTime( '@' . $value );
				$datetime->setTimezone( $site_tz );

				// Get the month, date, and year from the server $datetime that way if we change the timezone, we
				// don't end up changing the date.
				$month = $datetime->format( 'n' );
				$date  = $datetime->format( 'j' );
				$year  = $datetime->format( 'Y' );

				// Get the offset between the site timezone and UTC in seconds
				$offset_seconds = $site_tz->getOffset( $datetime );

				// Adjust the timestamp by the offset
				$datetime->modify( "-$offset_seconds seconds" );

				// Convert $datetime to UTC
				$datetime->setTimezone( new DateTimeZone( 'UTC' ) );

				// Subtract the number of minutes by the POSTed GMT offset
				$datetime->modify( '-' . rgpost( 'gpcld_visitor_gmt_offset' ) . ' minutes' );

				// Change the $datetime back to the $month, $date, and $year we stored from the server $datetime
				$datetime->setDate( $year, $month, $date );

				// Re-populate $value with the epoch from $datetime
		...
		/**
		 * Adds an init script to populate a hidden input with the visitor's browser timezone.
		 *
		 * @param array $form
		 ...
		 * @return void
		 */
		public function register_client_offset_init_script( $form, $field_values ) {
			if ( ! $this->should_use_client_offset( $form['id'] ) ) {
				return;
			}

			$script = <<<SCRIPT
			jQuery( '#gform_' + {$form['id']} + ' .gform_footer' )
				.append( '<input type="hidden" name="gpcld_visitor_gmt_offset" value="' + new Date().getTimezoneOffset() + '" />' );

			window['GPCLD_USE_VISITOR_TIMEZONE_{$form['id']}'] = true;
SCRIPT;

			GFFormDisplay::add_init_script( $form['id'], 'gpcld_add_client_offset_hidden_input', GFFormDisplay::ON_PAGE_RENDER, $script );
		}

		/**
		 * Helper method to determine if client offset is to be used for a given form ID.
		 *
		 * @param int $form_id
		 *
		 * @return bool
		 */
		public function should_use_client_offset( $form_id ) {
			/**
			 * By default, Conditional Logic Dates will use the server's timezone for conditional logic.
			 * Return `true` in a filter for this hook to use the visitor browser's timezone instead.
			 *
			 * @since 1.2.11
			 *
			 * @param bool  $enable    Use visitor's timezone instead of the server's timezone for conditional logic.
			 *                         Defaults to `false`.
			 * @param int   $form_id   The current form ID.
			 */
			return gf_apply_filters( array( 'gpcld_use_visitor_timezone', $form_id ), false, $form_id );
		}
			...
			foreach ( $object as $prop => $value ) {
				...
				if ( $prop && $prop == 'conditionalLogic' && ! empty( $value ) ) {
					foreach ( $object[ $prop ]['rules'] as $rule ) {
						...
						if ( in_array( $rule['fieldId'], array( '_gpcld_current_time', '_gpcld_current_date' ), true ) ) {
							// Create a field-like array for non-field-type rule field IDs. This helps them play nice
							// with other parts of Conditional Logic Dates where field objects are expected.
						...
						if ( $ruleField && in_array( GFFormsModel::get_input_type( $ruleField ), array( 'date', 'time' ) ) ) {
		...
		public static function convert_conditional_logic_rules( $rules, $form ) {
			...
			foreach ( $rules as &$rule ) {
				...
				foreach ( $matches as $match ) {
					...
					switch ( $tag ) {
						case 'today':
							...
							if ( $modifier ) {
								$modifier = str_replace( '%nextYear%', ( (int) date( 'Y' ) ) + 1, $modifier );
								$time     = strtotime( $modifier, current_time( 'timestamp' ) );
							} else {
				...
				// Deprecated but for backwards compatibility this accounts for conditions where a Date field is compared
				// to an empty rule value with the "greater than" or "less than" operators. Note: the `gf_matches_operation()`
				// JS function requires a values to be strings.
				elseif ( in_array( $rule['operator'], array( '<', '>' ) ) ) {
					$rule['value'] = '0';
				}
			}
